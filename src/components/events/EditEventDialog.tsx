...
export function EditEventDialog({ event, children, trigger }: EditEventDialogProps) {
  ...
  const { register, handleSubmit, watch, setValue, reset, formState: { errors } } = useForm<EventFormData>({
    defaultValues: {
      ...
      event_name_english: event.event_name_english || '',
      event_description: event.event_description || '',
      event_type: event.event_type,
      event_format: event.event_format,
      event_status: event.event_status,
      event_start_date: format(new Date(event.event_start_date), 'yyyy-MM-dd'),
      event_end_date: format(new Date(event.event_end_date), 'yyyy-MM-dd'),
      ...
      is_multi_track: event.is_multi_track,
      gps_validation_required: event.gps_validation_required,
      geofence_validation_required: event.geofence_validation_required,
  ...
  useEffect(() => {
    if (open) {
      // Reset form with event data when dialog opens
      reset({
        event_code: event.event_code,
        event_name: event.event_name,
        event_name_english: event.event_name_english || '',
  ...
  const updateMutation = useMutation({
    mutationFn: (data: EventFormData) => api.events.update(event.id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['events'] });
      toast({
        title: 'ជោគជ័យ',
        description: 'បានធ្វើបច្ចុប្បន្នភាពព្រឹត្តិការណ៍ដោយជោគជ័យ',
      });
      setOpen(false);
    },
    onError: (error: any) => {
      toast({
        title: 'បរាជ័យ',
        description: error.message || 'មានបញ្ហាក្នុងការធ្វើបច្ចុប្បន្នភាព',
        variant: 'destructive',
      });
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>កែសម្រួលព្រឹត្តិការណ៍</DialogTitle>
          <DialogDescription>
            ធ្វើបច្ចុប្បន្នភាពព័ត៌មានព្រឹត្តិការណ៍
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {(eventFormat === 'IN_PERSON' || eventFormat === 'HYBRID') && (
            <div className="space-y-4">
              ...
              <CascadingLocationSelector
                selectedProvinceName={event.province_name || undefined}
                selectedDistrictName={event.district_name || undefined}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    value={watch('virtual_platform') || ''}
                    onValueChange={(value) => setValue('virtual_platform', value)}
                  >
                    <SelectTrigger>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={watch('allow_public_registration')}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <Button type="submit" disabled={updateMutation.isPending}>
              {updateMutation.isPending ? 'កំពុងរក្សាទុក...' : 'រក្សាទុកការផ្លាស់ប្តូរ'}
            </Button>
          </div>
        </form>
      </DialogContent>
